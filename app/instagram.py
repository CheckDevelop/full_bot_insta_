...
class InstagramClient:
    def __init__(self, username: str, session_b64: str, temp_dir: Path, max_media_bytes: int, request_timeout_seconds: int = 60) -> None:
    ...
    def _build_loader(self) -> instaloader.Instaloader:
        ...
        if logged_in != self.username:
            raise InstagramAuthenticationError(
                f"Instagram session mismatch: expected {self.username}, got {logged_in!r}"
            )
    ...
    def _download_post(self, url: str) -> list[MediaFile]:
        ...
        try:
            ...
            if post.typename == "GraphImage":
                ...
                for index, node in enumerate(post.get_sidecar_nodes(), start=1):
                    if node.is_video:
                        if not node.video_url:
                            raise InstagramError("URL یکی از ویدیوهای carousel دریافت نشد.")
                ...
                raise InstagramError(f"نوع پست پشتیبانی نمی‌شود: {post.typename}")

            return result
        except instaloader.exceptions.LoginRequiredException as exc:
            raise InstagramAuthenticationError("Session اینستاگرام معتبر نیست یا نیاز به ورود مجدد دارد.") from exc
        except Exception:
    ...
    def _find_story_item(self, username: str, media_id: int):
        try:
            ...
            raise InstagramAuthenticationError("Session اینستاگرام برای استوری معتبر نیست.") from exc
        return None
    ...
    def _download_story(self, url: str) -> list[MediaFile]:
        ...
        self._download_url(item.video_url if item.is_video else item.url, path)
        return [MediaFile(path, "video" if item.is_video else "photo")]
    ...
    def _download_highlight(self, url: str) -> list[MediaFile]:
        ...
        try:
            response = session.get(api_url, headers=headers, timeout=(20, 60))
            response.raise_for_status()
            data = response.json()
            highlight = data.get("reels", {}).get(f"highlight:{highlight_id}")
            if not highlight:
                raise InstagramError("اطلاعات هایلایت پیدا نشد.")
            items = highlight.get("items", [])
        except requests.RequestException as exc:
            raise InstagramError("دسترسی به اطلاعات هایلایت ناموفق بود.") from exc
        except ValueError as exc:
            raise InstagramError("پاسخ اینستاگرام برای هایلایت معتبر نبود.") from exc

        job_dir = self._new_job_dir()
        result: list[MediaFile] = []
        username = str(highlight.get("user", {}).get("username") or "highlight")
        ...
        try:
            for index, item in enumerate(items, start=1):
                ...
                if media_type == 2:
                    ...
                    if not media_url:
                        continue
                    path = job_dir / f"{username}_highlight_{highlight_id}_{index}.mp4"
                    ...
                    if not media_url:
                        continue
                    path = job_dir / f"{username}_highlight_{highlight_id}_{index}.jpg"
                    self._download_url(media_url, path)
                    result.append(MediaFile(path, "photo"))

            if not result:
                raise InstagramError("هایلایت خالی است یا فایل قابل دانلود پیدا نشد.")
            return result
        except Exception:
            shutil.rmtree(job_dir, ignore_errors=True)
            raise
    ...
    def cleanup_files(files: list[MediaFile]) -> None:
        ...
        for media in files:
            try:
                ...
                logger.exception("Failed to remove file %s", media.path)
